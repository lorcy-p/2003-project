...
const ChatScreen: React.FC = () => {
  ...
  const handleSendMessage = () => {
    if (newMessage.trim() !== '') {
      setMessages([...messages, { sender: 'user', text: newMessage, timestamp: getCurrentTime() }]);
      setNewMessage(''); 
    }
  };
        ...
        <div className="messageContainer">
          {messages.map((message, index) => (
            <div
              key={index}
              className={`message ${
                message.sender === "recipient" ? "messageReceiver" : "messageSender"
              }`}
            >
              {message.sender === "recipient" && (
                <>
                  <div className="receiverName font-corpos">AI Model</div>
                  <div className="receiverMessage font-corpos">{message.text}</div>
                  {message.attachment && (
              ...
              <div
                className={
                  message.sender === "recipient" ? "timeReceiver" : "timeSender"
                }
              >
                {message.timestamp}
              </div>
